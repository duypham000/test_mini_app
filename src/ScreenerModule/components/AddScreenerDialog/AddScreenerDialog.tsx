...
const AddScreenerDialog: React.FC = (): JSX.Element => {
  ...
  const handleAddScreener = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    ;(async () => {
      try {
        ...
        dispatch(setRulesCreation(JSON.stringify([])))
        dispatch(setScreenerList(screenerList?.concat(res.data) as any))
        dispatch(setVisibleAddScreener(false))
  ...
  return (
    <DialogWrap
      ...
      visible={visibleAddScreener}
      title={
        <Typography variant='heading_three'>{`Tạo bộ lọc mới`}</Typography>
      }
      onClose={() => dispatch(setVisibleAddScreener(false))}
      ...
      <form onSubmit={handleAddScreener}>
        ...
        <div css={styles.btnGroup()}>
          <button
            ...
            onClick={() => dispatch(setVisibleAddScreener(false))}
          >
            <Typography
