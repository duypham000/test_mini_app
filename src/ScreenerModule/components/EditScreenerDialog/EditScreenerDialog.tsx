...
const EditScreenerDialog: React.FC = (): JSX.Element => {
  ...
  //   currentScreener?.isPublic || false
  // );

  /**
   * state
   */
  const [name, setName] = React.useState(currentScreener?.name || '')
  ...
  const handleAddScreener = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    ;(async () => {
      try {
        await screenerApi.updateScreener({
        ...
        dispatch(setVisibleEditScreener(false))
      } catch (error: any) {
  ...
  return (
    <DialogWrap
      ...
      visible={visibleEditScreener}
      title={
        <Typography variant='heading_three'>{`Chỉnh sửa bộ lọc`}</Typography>
      }
      onClose={() => dispatch(setVisibleEditScreener(false))}
      ...
      <form onSubmit={handleAddScreener}>
        ...
        <div css={styles.btnGroup()}>
          <button
            ...
            onClick={() => dispatch(setVisibleEditScreener(false))}
          >
            <Typography
...
export default EditScreenerDialog
